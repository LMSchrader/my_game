...
export class InteractionHandler {
    ...
    private handleCharacterClick(clickedCharacter: Character, selectedCharacter: Character | undefined): void {
        if (!this.isCharacterPlayable(clickedCharacter)) {
            logger.debug(`Cannot select ${clickedCharacter.name}: not active character or wrong team`)
            return
        }
    ...
    private handleEmptyTileClick(hex: HexCoordinates, selectedCharacter: Character | undefined): void {
        if (!selectedCharacter) {
            return
        }

        if (!this.isCharacterPlayable(selectedCharacter)) {
            logger.debug(`${selectedCharacter.name} cannot move: not active character or wrong team`)
            this.gameState.deselectCharacter()
            this.hexGrid.clearHighlights()
            return
        }
