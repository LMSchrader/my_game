...
export class TurnOrderDisplay extends Container {
  ...
  private characterCards: Map<string, Graphics> = new Map()
  private characterTexts: Map<string, Text> = new Map()
  ...
  private createCard(character: Character): Graphics {
    ...
    const teamColor = character.team === Team.TeamA ? 0x3b82f6 : 0xef4444

    card.roundRect(0, 0, this.cardWidth, this.cardHeight, 8)
    card.fill(teamColor)
    card.stroke({ width: 2, color: 0xffffff })
  ...
  public setActiveCharacter(characterId: string): void {
    ...
    if (activeCard) {
      ...
      activeCard.stroke({ width: 4, color: 0xffffff })
    }
